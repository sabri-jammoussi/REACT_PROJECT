...
export default function Listcategorie() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/sandwich").then((res) => {
      setcate(res.data);
  ...
  const handleDeletee=(id)=>{
    if (window.confirm("Voulez-vous supprimer ce sandwich ?") === true) {
      axios.delete("http://localhost:5000/sandwich/"+id).then((res) => {
  ...
  return (
    <div>
      <>
        <Button variant="primary"> {<Link to={"/categorieI"}style={{textDecoration:"none",color:"white"}} >Ajouter</Link>}</Button>{' '}
      </>
      <br />
      Listes des sandwich
            ...
            listcate.map((art, i) =>
              <tr key={art.id}>
                ...
                <td><Button variant="primary">
                    <Link to={`/categorieE/${art.id}`}style={{ textDecoration: "none", color: "white" }} >Modifier</Link></Button> 
